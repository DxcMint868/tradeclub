...
const NAV_MENU = [
  {
    label: "TRADE",
    items: [
      { label: "SWAP", desc: "Instant Execution", icon: Repeat },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "WATCH", desc: "Spectate Pros", icon: Eye },
      { label: "PREDICT", desc: "Market Outlook", icon: Dice5 },
    ],
  },
  { label: "LEADERBOARD", href: "/leaderboard", icon: Trophy },
  { label: "AI CHAT", href: "/ai-chat", icon: Cpu },
            ...
            <div key={i} className="relative group" onMouseEnter={() => setHoveredIndex(i)} onMouseLeave={() => setHoveredIndex(null)}>
              <button
                className={`cursor-pointer relative z-10 flex items-center gap-2 px-6 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest transition-all duration-300 ${
                  hoveredIndex === i ? "text-white" : "text-gray-400 hover:text-white"
                }`}
              >
                  ...
                  <div className="bg-[#0a0a0a] border border-white/10 p-2 rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.8)] backdrop-blur-xl relative overflow-hidden">
                    ...
                    <div className="flex flex-col gap-1">
                      {item.items.map((subItem, j) => {
                        const commonClass = "flex items-center gap-4 p-3 rounded-xl hover:bg-white/5 cursor-pointer group/item transition-colors";
                        const content = (
                          <>
                            <div className="p-2 bg-white/5 rounded-lg group-hover/item:bg-neon/20 group-hover/item:text-neon transition-colors text-gray-500">
                              {subItem.icon && <subItem.icon size={16} />}
                            </div>
                            <div>
                              <div className="text-sm font-bold text-gray-200 group-hover/item:text-white font-rajdhani">{subItem.label}</div>
